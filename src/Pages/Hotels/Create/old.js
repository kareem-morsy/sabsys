...
import { getHotelById, editHotel, createHotel } from "../../Services/Hotels";
...
const NewHotel = () => {
  ...
  let cityOptions = cities?.map((c) => ({ value: c.id, label: c.name }));
  ...
  const handleMealChange = (event, id) => {
    ...
    if (event.target.checked) {
      updatedList = [
        ...meals,
        { id: id, title: event.target.value, selling_price: 0 },
      ];
    } else {
      updatedList.splice(
        meals.indexOf(meals?.find((meal) => meal.title === event.target.value)),
        1
      );
    }
  ...
  const handleExtraChange = (event, id, extraType) => {
    ...
    if (event.target.checked) {
      ...
      updatedList.splice(
        extras.indexOf(
          extras.find((extra) => extra.title === event.target.value)
        ),
        1
      );
    }
